...
@pytest.mark.asyncio
async def test_device_user_greeting_includes_unread(device_user_metadata, mock_db, mock_env):
    """Greeting mentions unread message count."""
    from agents.device_user_agent import DeviceUserAgent
    agent = DeviceUserAgent(metadata=device_user_metadata, db=mock_db)
    ...
    text_events = [e for e in events if isinstance(e, AgentSendText)]
    greeting = text_events[0].text.lower()
    assert "1 new message" in greeting
...
async def test_unknown_caller_passphrase_correct(unknown_caller_metadata, mock_db, mock_env):
    ...
    # Correct passphrase
    events = []
    async for event in agent.process(mock_env, UserTextSent(content="sunflower garden")):
        events.append(event)
    text_events = [e for e in events if isinstance(e, AgentSendText)]
...
async def test_unknown_caller_passphrase_wrong(unknown_caller_metadata, mock_db, mock_env):
    ...
    # First wrong attempt
    events = []
    async for event in agent.process(mock_env, UserTextSent(content="wrong answer")):
        events.append(event)
    assert not any(isinstance(e, AgentEndCall) for e in events)

    # Second wrong attempt -> end call
    events = []
    async for event in agent.process(mock_env, UserTextSent(content="still wrong")):
        events.append(event)
...
async def test_anonymous_message_attribution(mock_db):
    ...
    mock_ctx = MagicMock()
    await leave_anonymous.func(mock_ctx, message_content="Surprise!")
